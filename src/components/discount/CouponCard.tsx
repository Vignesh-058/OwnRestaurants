    ...
    <>
      <div className={`rounded-3xl overflow-hidden border shadow-sm hover:shadow-xl transition-all duration-300 group ${isExpired ? 'opacity-60' : ''}`}>
        ...
        <div className={`bg-gradient-to-r ${gradient} p-5 relative overflow-hidden`}>
          ...
          <div className="relative z-10">
            ...
            <div className="inline-flex items-center gap-2 border-2 border-dashed border-white/50 rounded-xl px-3 py-1.5 bg-white/10">
              ...
              <span className="font-black text-white text-sm tracking-widest uppercase">{coupon.code}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card p-5">
          ...
          {coupon.description && (
            <p className="text-sm text-muted-foreground mb-4 leading-relaxed line-clamp-2">{coupon.description}</p>
          )}
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              className="flex-1 rounded-full font-bold border-[#FFE2CC] text-[#FF6B00] hover:bg-[#FFF4EB]"
              onClick={() => setIsDetailsOpen(true)}
            >
              View Details
            </Button>
            <Button
              className={`flex-1 rounded-full font-bold transition-all duration-300 ${isApplied ? 'bg-emerald-600 hover:bg-emerald-700 text-white' : ''}`}
