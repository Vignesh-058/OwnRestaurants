...
const getCategoryIcon = (name: string) => {
  ...
  if (
    ...
    return Flame;
  if (n.includes("coffee") || n.includes("tea") || n.includes("beverage"))
    return Coffee;
  ...
  if (
    ...
    return Croissant;
  if (n.includes("rice") || n.includes("biryani") || n.includes("meal"))
    return Wheat;
...
export const ProductCatalogue = ({
  ...
  const filteredCategories = categories.filter((c) => {
    ...
    const catName =
      (c as any).categoryName || c.name || (c as any).displayName || "Unknown";
  ...
  return (
    <div className="w-full relative z-10 flex flex-col bg-[var(--background)]">
      ...
      <section
        id="product-menu"
        className="w-full flex flex-col lg:flex-row relative z-10 pt-2 lg:pt-4 px-0"
      >
        {/* LEFT: Premium Sidebar Categories */}
        <aside
          className={cn(
            "shrink-0 z-40 sticky top-[60px] lg:top-[80px] lg:h-[calc(100vh-80px)] bg-[var(--background)] mb-4 lg:mb-0 transition-all duration-300 overflow-hidden flex flex-col shadow-sm lg:shadow-none pb-3 lg:pb-0 border-b border-border lg:border-none",
            isSidebarOpen
              ? "w-full lg:w-[300px] xl:w-[340px] opacity-100"
              : "w-full lg:w-0 lg:opacity-0 lg:m-0",
          )}
        >
          <div className="flex flex-row lg:flex-col lg:flex-1 overflow-x-auto lg:overflow-y-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent gap-3 lg:gap-2 px-4 lg:px-0 pb-2 lg:pb-4 pt-1 lg:pt-2 snap-x min-w-full lg:min-w-[300px] xl:min-w-[340px]">
                    ...
                    <div
                      className={cn(
                        ...
                        isActive
                          ...
                          : "bg-[var(--background)] text-primary group-hover:bg-white",
                      )}
                    >
                      {category.imageUrl ? (
                        <img
            ...
            {/* Search Bar - Immediate below categories */}
            <div className="px-4 lg:px-2 pt-2 pb-4 bg-[var(--background)] shrink-0 w-full">
              <div className="relative w-full">
                ...
                <input
                  ...
                  className="w-full h-11 bg-muted/50 rounded-xl pl-10 pr-4 text-sm font-medium outline-none border border-border/50 focus:border-primary/50 focus:bg-background transition-all"
                />
              </div>
            </div>
              ...
              {filters.filteredItems.length > 0 ? (
                ...
                <div className="py-24 text-center flex flex-col items-center justify-center bg-white rounded-3xl border border-dashed border-border shadow-sm mt-4">
                  ...
                  <Button
                    ...
                    className="mt-6 bg-primary hover:bg-primary/90 text-white rounded-full px-8 h-[48px] font-bold"
                  >
                    Clear Filters
                  </Button>
                </div>
