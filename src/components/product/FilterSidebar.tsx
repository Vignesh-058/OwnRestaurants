...
export const FilterSidebar = ({ categories, activeCategoryId, onSelectCategory, onOpenFilters, className }: FilterSidebarProps) => {
  ...
  return (
    <div 
      className={cn(
        "flex flex-col bg-white h-full w-full shrink-0 rounded-[16px] shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-border/60 overflow-hidden",
        className
      )}
    >
      <div className="flex flex-col h-full">
        ...
        <div className="p-5 pb-4 border-b border-border/40">
          <div className="flex items-start justify-between">
            <div>
              <h2 className="text-[22px] font-bold text-foreground leading-tight">Categories</h2>
              <p className="text-muted-foreground text-[13px] mt-0.5 font-medium">Browse Menu</p>
            </div>
            
            {/* New Filter Button */}
            <button 
              onClick={onOpenFilters}
              className="group flex items-center justify-center w-10 h-10 rounded-[12px] bg-accent border border-[rgba(255,107,0,0.15)] shadow-[0_4px_10px_rgba(255,107,0,0.08)] hover:bg-primary hover:scale-105 hover:shadow-[0_4px_12px_rgba(255,107,0,0.2)] transition-all duration-250 cursor-pointer"
              title="Open Filters"
            >
              <SlidersHorizontal className="w-5 h-5 text-primary group-hover:text-white transition-colors duration-250" />
            </button>
          </div>
          ...
          <div className="relative mt-5">
            ...
            <Input 
              ...
              className="pl-10 h-[46px] bg-muted/80 border-border rounded-[14px] text-[14px] focus-visible:ring-2 focus-visible:ring-primary/20 focus-visible:border-primary placeholder:text-muted-foreground transition-all"
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    isActive 
                      ? "bg-primary text-white shadow-[0_4px_12px_rgba(255,107,0,0.25)] scale-[1.02] ml-1 mr-1 w-[calc(100%-8px)]" 
                      : "bg-transparent hover:bg-accent hover:-translate-y-0.5 text-foreground"
                  )}
                >
                  <div className="flex items-center gap-3 overflow-hidden">
                    <div className={cn(
                      "w-10 h-10 rounded-[10px] overflow-hidden shrink-0 flex items-center justify-center transition-colors shadow-sm",
                      isActive ? "bg-white/20" : "bg-white border border-border/60"
                    )}>
                      <img 
                        src={iconSrc} 
                        alt={name} 
                  ...
                  <div className={cn(
                    ...
                    isActive 
                      ...
                      : "bg-white border border-border text-muted-foreground group-hover:border-primary/30 group-hover:text-primary"
                  )}>
                    {count}
                  </div>
                </button>
