...
export const PriceSummary = ({
 ...
 return (
 <div className="bg-muted p-4 sm:p-5 flex flex-col gap-4 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] border-t border-muted rounded-t-[24px] sm:rounded-t-[32px] relative z-20">
 {!inStock && (
 ...
 <div className={cn(
 "flex items-center bg-foreground rounded-[16px] h-[56px] px-2 shrink-0 transition-opacity",
 !inStock && "opacity-50 pointer-events-none"
 )}>
 <button
 onClick={() => onQuantityChange(Math.max(1, quantity - 1))}
 disabled={quantity <= 1 || !inStock}
 className="w-[40px] h-[40px] flex items-center justify-center rounded-[12px] text-muted-foreground hover:text-white hover:bg-foreground disabled:opacity-50 transition-colors"
 >
