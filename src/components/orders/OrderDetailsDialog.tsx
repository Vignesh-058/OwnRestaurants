...
import { MapPin, User, Phone, Package, CreditCard, Tag, X, RotateCcw, Loader2 } from 'lucide-react';
...
export const OrderDetailsDialog = ({ order, open, onClose }: OrderDetailsDialogProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="fixed right-0 md:top-[5%] md:bottom-auto bottom-0 md:left-1/2 md:-translate-x-1/2 w-full md:w-[75%] lg:w-[70%] max-h-[90vh] h-[90vh] p-0 flex flex-col overflow-hidden md:rounded-2xl rounded-t-[20px] rounded-b-none border-none shadow-[0_10px_40px_rgba(0,0,0,0.1)] m-0 bg-background">
        
        {/* Sticky Header */}
        <div className="flex-none bg-white px-5 py-4 border-b border-border/50 z-10 sticky top-0 flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <DialogTitle className="text-[20px] font-black text-foreground">Order #{orderId?.slice(-8).toUpperCase()}</DialogTitle>
              {order.orderStatus && <OrderStatusBadge status={order.orderStatus as any} />}
          ...
          <button onClick={onClose} className="h-8 w-8 rounded-full bg-muted/50 hover:bg-muted flex items-center justify-center transition-colors">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {/* Timeline Section */}
              <div className="bg-white p-5 rounded-2xl shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-border/50">
                <h4 className="font-extrabold text-[14px] text-foreground mb-5 uppercase tracking-wider">Order Timeline</h4>
                <OrderTimeline status={order.orderStatus as any} />
              </div>

              {/* Items Section */}
              <div className="bg-white p-5 rounded-2xl shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-border/50">
                <h4 className="font-extrabold text-[14px] text-foreground mb-4 uppercase tracking-wider flex items-center gap-2">
                  <Package className="h-4 w-4 text-primary" /> Items Ordered ({itemCount})
                </h4>
                <div className="space-y-3">
                  {order.items?.map((item: any, idx: number) => {
                    ...
                    return (
                      <div key={item.itemid ?? item._id ?? idx} className="flex items-start gap-4 p-3 rounded-[12px] border border-border/50 hover:border-border bg-background/50 transition-colors">
                        {image && (
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {item.variationName && (
                            <p className="text-[12px] font-semibold text-muted-foreground mt-0.5">{item.variationName}</p>
                          )}
                          {((item.addOnDetails || item.addons) ?? []).length > 0 && (
                            <div className="mt-2 flex flex-wrap gap-1.5">
                              {(item.addOnDetails || item.addons)?.map((a: any, ai: number) => (
                                <span key={ai} className="text-[10px] font-bold text-muted-foreground bg-white border border-border px-2 py-1 rounded-[6px]">
                          ...
                          {order.instructions && (
                             <p className="text-[11px] font-medium text-primary mt-2 italic bg-primary/10 p-1.5 rounded-[6px]">
                               Note: {order.instructions}
                             </p>
                          )}
                        </div>
                        <div className="text-right shrink-0 flex flex-col items-end">
                          <p className="text-[14px] font-black text-foreground">₹{price.toLocaleString()}</p>
                          <span className="text-[11px] font-bold text-muted-foreground bg-muted/50 px-2 py-0.5 rounded-[4px] mt-1">
                            Qty ×{item.quantity}
                          </span>
            ...
            <div className="space-y-6">
              ...
              {/* Customer Info Card */}
              <div className="bg-white p-5 rounded-2xl shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-border/50 space-y-4">
                <h4 className="font-extrabold text-[14px] text-foreground uppercase tracking-wider flex items-center gap-2">
                ...
                <div className="space-y-4">
                  ...
                  {address && (
                    <div className="flex items-start gap-3 mt-2">
                      ...
                      <div>
                        <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Delivery Address</p>
              ...
              <div className="bg-white p-5 rounded-2xl shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-border/50">
                <h4 className="font-extrabold text-[14px] text-foreground mb-4 uppercase tracking-wider flex items-center gap-2">
                  <Tag className="h-4 w-4 text-primary" /> Bill Summary
                </h4>
                <div className="space-y-3">
                  {(() => {
                    ...
                    return (
                      <>
                        ...
                        {discount !== undefined && discount !== null && discount > 0 && (
                          <div className="flex justify-between items-center text-[13px]">
                            <span className="font-bold text-success">Coupon Discount</span>
                        ...
                        {packageCharge !== undefined && packageCharge !== null && packageCharge > 0 && (
                          <div className="flex justify-between items-center text-[13px]">
                            <span className="font-medium text-muted-foreground">Packing Charge</span>
                            <span className="font-bold text-foreground">₹{packageCharge.toLocaleString()}</span>
                          </div>
                        )}
                        ...
                        {deliveryCharge !== undefined && deliveryCharge !== null && (
                          <div className="flex justify-between items-center text-[13px]">
                            ...
                            <span className="font-bold text-foreground">₹{deliveryCharge.toLocaleString()}</span>
                          </div>
                        )}
        ...
        {/* Sticky Footer */}
        <div className="flex-none bg-white p-4 md:px-6 md:py-4 border-t border-border/50 z-10 sticky bottom-0">
          <div className="flex justify-between items-center">
            {total !== undefined && total !== null ? (
              <div>
                <p className="text-[12px] font-bold text-muted-foreground uppercase tracking-wider mb-0.5">Total Amount</p>
                <p className="text-[24px] font-black text-primary leading-none">₹{total.toLocaleString()}</p>
              </div>
            ) : (
               <div />
            )}
      ...
      <AlertDialog open={showConfirmDialog} onOpenChange={cancelReorder}>
        <AlertDialogContent className="rounded-3xl">
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => { confirmReorder(); onClose(); }} className="bg-primary hover:bg-primary rounded-full font-bold text-white">
              Switch & Reorder
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
