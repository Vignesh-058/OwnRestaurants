...
export const useProductDetail = (itemId: string | null, isOpen: boolean) => {
  ...
  const basePrice = useMemo(() => {
    ...
    if (selectedVariation && item.variations) {
      const v = item.variations.find((v: any) => v.variationid === selectedVariation);
      if (v) return v.price || v.sellingPrice || v.basePrice || item.sellingPrice || item.basePrice || 0;
    }
    return item.sellingPrice || item.basePrice || 0;
  }, [item, selectedVariation]);
  ...
  const addonsPrice = useMemo(() => {
    ...
    Object.entries(selectedAddons).forEach(([groupId, itemIds]) => {
      const group = item.addons.find((g: any) => g.addongroupid === groupId);
      if (group) {
        itemIds.forEach(id => {
          const addonItem = group.items.find((i: any) => i.addonitemid === id);
  ...
  const isValid = useMemo(() => {
    ...
    if (item.stockStatus === false || item.inStock === false) return false;
    
    // Check addon constraints
    if (item.addons) {
      for (const group of item.addons) {
        const selectedCount = (selectedAddons[group.addongroupid] || []).length;
        if (selectedCount < group.min || ((group.max || 0) > 0 && selectedCount > group.max)) {
          return false;
        }
      }
    }
